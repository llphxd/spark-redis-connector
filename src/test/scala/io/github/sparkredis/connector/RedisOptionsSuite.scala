...
class RedisOptionsSuite extends AnyFunSuite {
  test("parses defaults") {
    ...
    assert(options.connectTimeoutMillis == 2000)
    assert(options.socketTimeoutMillis == 2000)
    assert(options.poolMaxTotal == 8)
    assert(options.poolMaxIdle == 8)
    assert(options.poolMinIdle == 0)
    assert(!options.sslEnabled)
    assert(options.writeBatchSize == 1000)
  ...
  test("parses case-insensitive options") {
    ...
    raw.put("CONNECT.TIMEOUT", "3000")
    raw.put("SOCKET.TIMEOUT", "4000")
    ...
    raw.put("WRITE.BATCH.SIZE", "250")
    raw.put("PIPELINE.SIZE", "100")
    ...
    assert(options.connectTimeoutMillis == 3000)
    assert(options.socketTimeoutMillis == 4000)
    ...
    assert(options.writeBatchSize == 250)
